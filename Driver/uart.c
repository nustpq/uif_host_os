...
CPU_INT08U UART_Init( CPU_INT08U uart_index,void (*isr_handler)( void ),CPU_INT32U baud )
{
    ...
    if( uart_index < 3 ) { //for USART0  USART1  USART2  
        ...
        AT91C_BASE_PMC->PMC_PCER      = 1 << UART_PID[uart_index];
...
{    
    ...
    AT91C_BASE_PIOA->PIO_PDR  |= (DEF_BIT_30|DEF_BIT_31);                 /* Set GPIOA pins 9 and 10 as DBGU UART pins                */
    AT91C_BASE_PIOA->PIO_ASR  |= (DEF_BIT_30|DEF_BIT_31);                 /* Select GPIOA attached peripheral (DBGU)                  */
